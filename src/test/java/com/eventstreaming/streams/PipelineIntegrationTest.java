...
/**
 * Integration test for the complete Pekko Streams pipeline.
 */
...
class PipelineIntegrationTest {
    ...
    void setUp() {
        ...
        // Create real metrics with simple registry
        SimpleMeterRegistry meterRegistry = new SimpleMeterRegistry();
        metrics = new StreamProcessingMetrics(actorSystem, meterRegistry);
        
        // Mock sequence validator to always return valid
        EventSequenceValidator.SequenceValidationResult validResult = 
    ...
    void testMetricsIntegration() {
        // Given
        StreamProcessingMetrics.MetricsSummary initialSummary = metrics.getMetricsSummary();
        
        // When
        metrics.incrementEventsProcessed();
        metrics.incrementEventsFailed();
        metrics.recordEventProcessingTime(java.time.Duration.ofMillis(100));
        
        // Then
        StreamProcessingMetrics.MetricsSummary updatedSummary = metrics.getMetricsSummary();
        
        assertEquals(initialSummary.eventsProcessed() + 1, updatedSummary.eventsProcessed());
        assertEquals(initialSummary.eventsFailed() + 1, updatedSummary.eventsFailed());
        assertTrue(updatedSummary.avgProcessingTimeMs() > 0);
    }
